...
int main(int argc, char **argv)
{
   ...
   */
  double **E, **R, **E_prev;
  double **device_E = 0, **device_R = 0, **device_E_prev = 0;
  ...
  int blocksx = n / block_size.x;
  if (n % block_size.x != 0)
    blocksx++;
  int blocksy = n / block_size.y;
  if (n % block_size.y != 0)
    blocksy++;
  const dim3 num_blocks(blocksx, blocksy);
  size_t sizeofarray = sizeof(double *) * (m + 2) + sizeof(double) * (n + 2) * (m + 2);
  cudaMalloc((void **)&device_E, sizeofarray);
  cudaMalloc((void **)&device_E_prev, sizeofarray);
  cudaMalloc((void **)&device_R, sizeofarray);
  cudaMemcpy(device_E, E, sizeofarray, cudaMemcpyHostToDevice);
  cudaMemcpy(device_E_prev, E_prev, sizeofarray, cudaMemcpyHostToDevice);
  cudaMemcpy(device_R, R, sizeofarray, cudaMemcpyHostToDevice);
  int init = 0;
  ...
  while (t < T)
  {
    ...
    if (kernel == 1)
    {
      karrayinit<<<num_blocks, block_size>>>(device_E, device_E_prev, device_R, n);                                     // printf("initerror: %s\n", cudaGetErrorString(cudaGetLastError()));
      k1halos<<<num_blocks, block_size>>>(device_E_prev, n, m);                                                         // printf("haloserror: %s\n", cudaGetErrorString(cudaGetLastError()));
      k1pde<<<num_blocks, block_size>>>(device_E, device_E_prev, alpha, n, m);                                          // printf("pdeerror: %s\n", cudaGetErrorString(cudaGetLastError()));
      k1ode<<<num_blocks, block_size>>>(device_E, device_E_prev, device_R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b); // printf("odeerror: %s\n", cudaGetErrorString(cudaGetLastError()));
      // kswap<<<1,1>>>(device_E, device_E_prev, device_R);
      double **tmp = device_E;
      device_E = device_E_prev;
      device_E_prev = tmp;
    }
    else if (kernel == 2)
    {
      k2<<<num_blocks, block_size>>>(device_E, device_E_prev, device_R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b, init); // printf("error: %s\n", cudaGetErrorString(cudaGetLastError()));
      init = 1;
      double **tmp = device_E;
      device_E = device_E_prev;
      device_E_prev = tmp;
    ...
    {
      k3<<<num_blocks, block_size>>>(device_E, device_E_prev, device_R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b, init); // printf("error: %s\n", cudaGetErrorString(cudaGetLastError()));
      init = 1;
      double **tmp = device_E;
      device_E = device_E_prev;
      device_E_prev = tmp;
    ...
    {
      k4<<<num_blocks, block_size, (sizeof(double) * (bx + 2) * (by + 2))>>>(device_E, device_E_prev, device_R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b, init, bx, by); 
      //printf("error: %s\n", cudaGetErrorString(cudaGetLastError()));
      init = 1;
      double **tmp = device_E;
      device_E = device_E_prev;
    ...
    if (plot_freq)
    {
      ...
      if ((t - k * plot_freq) < dt)
      {
        if (kernel != 0)
        {
          cudaMemcpy(E, device_E, sizeofarray, cudaMemcpyDeviceToHost);
          // printf("error: %s\n", cudaGetErrorString(cudaGetLastError()));
          for (j = 0; j < n + 2; j++)
          {
            E[j] = (double *)(E + n + 2) + j * (n + 2);
          }
        }
  ...
  if (kernel != 0)
  {
    cudaMemcpy(R, device_R, sizeofarray, cudaMemcpyDeviceToHost);
    cudaMemcpy(E, device_E, sizeofarray, cudaMemcpyDeviceToHost);
    cudaMemcpy(E_prev, device_E_prev, sizeofarray, cudaMemcpyDeviceToHost);
    // printf("before cudamemcpy:%s\n", cudaGetErrorName(cudaGetLastError()));
    for (j = 0; j < n + 2; j++)
    {
      E[j] = (double *)(E + n + 2) + j * (n + 2);
      E_prev[j] = (double *)(E_prev + n + 2) + j * (n + 2);
      R[j] = (double *)(R + n + 2) + j * (n + 2);
    }
  }

  cudaFree(device_E);
  cudaFree(device_E_prev);
  cudaFree(device_R);
